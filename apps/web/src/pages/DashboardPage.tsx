...
export function DashboardPage() {
  ...
  return <div className="space-y-6">
    ...
    <Card><CardContent className="flex flex-col items-start justify-between gap-4 pt-6 sm:flex-row sm:items-center"><div><p className="text-sm text-muted-foreground">Tổng phí đã phát hành</p><p className="text-3xl font-bold text-orange-600">{money(stats?.totalPremium)}</p></div><div className="flex gap-2"><Button asChild><Link to="/policies/new">Tạo đơn</Link></Button></div></CardContent></Card>
  </div>;
}
